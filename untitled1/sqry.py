...
def toParts(img, eps, minsize):
	...
	if (reqs):
		for i in range(4):
			statmean = ImageStat.Stat(reqs[i]).mean
			delta += ((statmean[0] - m[0]) ** 2 +
					  (statmean[1] - m[1]) ** 2 +
					  (statmean[2] - m[2]) ** 2) ** 0.5
			if (delta > eps):
				flag = False
				break
	...
	if (flag):
		...
		ident = int((m[0]-1) * 256 ** 2 + m[1] * 256 + m[2])
		img = Image.open(c.select("examples", ident)[0][1])
